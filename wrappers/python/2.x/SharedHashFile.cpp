...
uint32_t shf_qs;
uint32_t shf_q_items;
uint32_t shf_q_item_size;
void* shf_q_pointer;
...
static PyObject *
SharedHashFile_debugVerbosityMore(
    PyObject *self, PyObject *args
    /* none */)
{
  sharedFileHash.DebugVerbosityMore();
  return Py_BuildValue("i", 0);
}
    ...
    /*char * queueName*/)
{
  ...
  int qid = sharedFileHash.QGetName(queueName, strlen(queueName));
...
{
  uint32_t qs, q_items, q_item_size, qids_nolock_max;

  if (!PyArg_ParseTuple(args, "IIII:qNew", &qs, &q_items, &q_item_size, &qids_nolock_max))
  {
    return NULL;
  }
  ...
  shf_qs = qs;
  shf_q_items = q_items;
  shf_q_item_size = q_item_size;
  shf_q_pointer = sharedFileHash.QNew(shf_qs, shf_q_items, shf_q_item_size, qids_nolock_max);

  Py_buffer* buff = new Py_buffer;
  PyBuffer_FillInfo(buff, NULL, shf_q_pointer, shf_qs * shf_q_items * shf_q_item_size, 0, PyBUF_CONTIG);
  PyObject *obj = PyMemoryView_FromBuffer(buff);
  return obj;
}
...
SharedHashFile_qGet(
    ...
    /* no args*/)
{
  void* pointer = sharedFileHash.QGet();

  Py_buffer* buff = new Py_buffer;
  PyBuffer_FillInfo(buff, NULL, pointer, shf_qs * shf_q_items * shf_q_item_size, 0, PyBUF_CONTIG);
  PyObject *obj = PyMemoryView_FromBuffer(buff);
  return obj;
}
...
static PyObject *
SharedHashFile_qPushHead(
    PyObject *self, PyObject *args
    /*uint32_t qid, uint32_t qiid*/)
{
    uint32_t qid, qiid;

  if (!PyArg_ParseTuple(args, "II:qPushHead", &qid, &qiid))
  {
    return NULL;
  }
    
    sharedFileHash.QPushHead(qid, qiid);
    return Py_BuildValue("i", 0); /* returns void */
}

static PyObject *
SharedHashFile_qPullTail(
    PyObject *self, PyObject *args
    /*uint32_t qid*/)
{
    uint32_t qid;

  if (!PyArg_ParseTuple(args, "I:qPullTail", &qid))
  {
    return NULL;
  }

  uint32_t index = sharedFileHash.QPullTail(qid);
  return Py_BuildValue("I", index);  
}

static PyObject *
SharedHashFile_qPullTailData(
    PyObject *self, PyObject *args
    /*uint32_t qid*/)
{
    uint32_t qid;
  ...
  /* Pull tail and get qiid */
  uint32_t index = sharedFileHash.QPullTail(qid);
  
  if (index == SHF_QIID_NONE)
    return Py_BuildValue(""); /* return None */
  else
  {
    PyObject *index_obj = Py_BuildValue("I", index);  

    /* get shf_qiid_addr pointer to element pulled off tail */
    void *pointer = (void *)shf_qiid_addr;
    Py_buffer* buff = new Py_buffer;
    PyBuffer_FillInfo(buff, NULL, pointer, shf_qiid_addr_len, 0, PyBUF_CONTIG);
    PyObject *pointer_obj = PyMemoryView_FromBuffer(buff);
    
    PyObject *list = PyList_New(2);
    PyList_SetItem(list, 0, pointer_obj);
    PyList_SetItem(list, 1, index_obj);
    return list;    
  }
}
...
static PyMethodDef SharedHashFileMethods[] = {
  ...
  {"debugVerbosityMore",  SharedHashFile_debugVerbosityMore, METH_VARARGS,
   "Set maximal Debug level."},
  ...
  {"qNew",  SharedHashFile_qNew, METH_VARARGS,
    "Make a new queue."},
  {"qGet",  SharedHashFile_qGet, METH_VARARGS,
    "Get queue."},
  ...
  {"qPushHead",  SharedHashFile_qPushHead, METH_VARARGS,
    "Push to head."},
  {"qPullTail",  SharedHashFile_qPullTail, METH_VARARGS,
    "Pull tail."},
  {"qPullTailData",  SharedHashFile_qPullTailData, METH_VARARGS,
    "Pull tail, returns data rather than index."},
